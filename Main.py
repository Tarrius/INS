...
class ModelManager:
    ...
    def __init__(self, args, data, pretrained_model=None):
        ...
        if pretrained_model is None:
            ...
            #pretrained_model = BertForModel.from_pretrained(args.bert_model, num_labels = data.n_unknown_cls)
        ...
        self.criterion_cluster = ClusterLoss(self.num_labels, args.cluster_temperature, self.device).to(
            self.device)
        self.criterion_hcr = HCR().to(self.device)
        self.criterion_scl = SupConLoss().to(self.device)
    ...
    def tsne_visualization(self,x,y,predicted):
        ...
        path = args.save_results_path
        TSNE_visualization(x, y, label_list, os.path.join(path, "base.pdf"))
    ...
    def visualize_training(self, args, data):
        ...
        total_features = torch.empty((0, 15)).to(self.device)
        total_labels = torch.empty(0, dtype=torch.long).to(self.device)
        total_logits = torch.empty((0, self.num_labels)).to(self.device)
        ...
        for batch in tqdm(eval_dataloader, desc="evaluation"):
            ...
            input_ids, input_mask, segment_ids, label_ids = batch
            with torch.no_grad():
                c, feat = self.model.forward_cluster(batch)
                total_features = torch.cat((total_features, c))
                total_labels = torch.cat((total_labels, label_ids))
                total_logits = torch.cat((total_logits, c))
        ...
        self.tsne_visualization(x_feats, y_true, y_pred)
    ...
    def train(self, args, data):
        ...
        # self.training_process_eval(args, data, e_step)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(data.train_unlabeled_dataloader, desc="Pseudo-Training")):
                ...
                self.model.eval()
                with torch.no_grad():
                    c, _ = self.model.forward_logits((input_ids_1, input_mask_1, segment_ids_1, label_ids_1))
                self.model.train()
  

                z_i, z_j, c_i, c_j = self.model(batch, mode='contrastive-clustering')
                #pdb.set_trace()
                loss_scl = self.criterion_scl(torch.cat((z_i,z_j),dim=0), c)
                f = torch.cat((z_i,z_j),dim=0)
                h = torch.cat((c_i,c_j),dim=0)
                loss_hcr = self.criterion_hcr(h,f).to('cuda')
                ...
                loss = loss_instance + loss_cluster + 5*loss_hcr + 0.01*loss_scl
            ...
            eval_acc = manager.evaluation(args, data)['ACC']
            print(f"eval_acc:{eval_acc.item()}")
    ...
    def save_model(self, args):
        ...
        # for name, param in self.save_model.named_parameters():
